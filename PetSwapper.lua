  ...
  if db.isEnabled and not swapping then
    ...
    for slot = 1, 3 do
      ...
      if petGUID then
        ...
        if (level < db.swapMin or level >= db.swapMax or health < 1) and ((slot == 1 and db.slotOne) or (slot == 2 and db.slotTwo) or (slot == 3 and db.slotThree)) then
          for index = 1, select(2, C_PetJournal.GetNumPets()) do
            if not swapped[slot] then
              ...
              local _health = C_PetJournal.GetPetStats(_petGUID)
              if _favorite and _level >= db.swapMin and _level < db.swapMax and _health > 0 then
